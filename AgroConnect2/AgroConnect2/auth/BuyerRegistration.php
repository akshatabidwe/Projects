...
if (isset($_POST['register'])) {
    ...
    if (!preg_match('/^\d{10}$/', $phonenumber)) {
        ...
        exit(); // Exit registration process if phone number is invalid
    }

    // Validate bank account number format (digits, desired length)
    if (!preg_match('/^\d{9,18}$/', $account)) {
        echo "<script>alert('Invalid Bank Account Number. Please enter a valid account number (9-18 digits).');</script>";
        exit(); // Exit registration process if account number is invalid
    }

    // Validate PAN number format
    if (!preg_match('/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/', $pan)) {
        echo "<script>alert('Invalid PAN Number. Please enter a valid PAN number in the format ABCDE1234F.');</script>";
        exit(); // Exit registration process if PAN number is invalid
    }

    // Check if password and confirm password match
    if ($password === $confirmpassword) {
        $query = "INSERT INTO buyerregistration (buyer_name, buyer_phone, buyer_addr, buyer_comp, buyer_license, buyer_bank, buyer_pan, buyer_mail, buyer_username, buyer_password) VALUES ('$name', '$phonenumber', '$address', '$company_name', '$license', '$account', '$pan', '$mail', '$username', '$encryption')";
